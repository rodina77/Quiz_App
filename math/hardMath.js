...
function loadQuiz() {
  fetch("/math/mathQuiz.json")
    ...
    .then((data) => {
      ...
      const drafts = JSON.parse(localStorage.getItem('hardDrafts')) || [];

      data.HardMathematics.forEach(res => {
...
function draft(buttonElement, name, question, answer1, answer2, answer3) {
  let drafts = JSON.parse(localStorage.getItem('hardDrafts')) || [];
  ...
  localStorage.setItem('hardDrafts', JSON.stringify(drafts));
