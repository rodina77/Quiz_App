...
function loadQuiz() {
  fetch("/math/mathQuiz.json")
    ...
    .then((data) => {
      ...
      const drafts = JSON.parse(localStorage.getItem('mediumDrafts')) || [];

      data.IntermediateMathematics.forEach(res => {
...
function draft(buttonElement, name, question, answer1, answer2, answer3) {
  let drafts = JSON.parse(localStorage.getItem('mediumDrafts')) || [];
  ...
  localStorage.setItem('mediumDrafts', JSON.stringify(drafts));
...
function displayDraftQuestions(originalScore) {
  ...
  if (drafts.length === 0) {
      ...
      setTimeout(() => {
      window.location.href = "/mathLevel.html";
      }, 3000);
  }
