...
function loadQuiz() {
  fetch("/physics/physicsQuiz.json")
    .then((response) => response.json())
    .then((data) => {
      const resultContainer = document.getElementById("result");
      const drafts = JSON.parse(localStorage.getItem('mediumPhyDrafts')) || [];

      data.IntermediatePhysics.forEach(res => {
...
function draft(buttonElement, name, question, answer1, answer2, answer3) {
  let drafts = JSON.parse(localStorage.getItem('mediumPhyDrafts')) || [];
  ...
  localStorage.setItem('mediumPhyDrafts', JSON.stringify(drafts));
  ...
  Swal.fire({
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        Swal.fire(`Your score: ${score} out of 20 and your badge is ${overallBadge}`)
          .then(() => {
            // Redirect after displaying the message
            setTimeout(() => {
            window.location.href = "/options_page/physics/index.html";
            }, 3000);
          });
      }
    });
...
function submitDraftQuestions(originalScore) {
  ...
  Swal.fire(`Your total score: ${totalScore} out of 20 and your badge is ${badge}`);
  setTimeout(() => {
  window.location.href = "/options_page/physics/index.html";
  }, 3000);
}
