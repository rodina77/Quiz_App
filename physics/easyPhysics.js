...
function loadQuiz() {
  fetch("/physics/physicsQuiz.json")
    ...
    .then((data) => {
      ...
      const drafts = JSON.parse(localStorage.getItem('easyPhyDrafts')) || [];

      data.EasyPhysics.forEach(res => {
...
function draft(buttonElement, name, question, answer1, answer2, answer3) {
  let drafts = JSON.parse(localStorage.getItem('easyPhyDrafts')) || [];
  ...
  localStorage.setItem('easyPhyDrafts', JSON.stringify(drafts));
