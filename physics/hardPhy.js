...
function loadQuiz() {
  fetch("/physics/physicsQuiz.json")
    ...
    .then((data) => {
      ...
      const drafts = JSON.parse(localStorage.getItem('hardPhyDrafts')) || [];

      data.HardPhysics.forEach(res => {
...
function draft(buttonElement, name, question, answer1, answer2, answer3) {
  let drafts = JSON.parse(localStorage.getItem('hardPhyDrafts')) || [];
  ...
  localStorage.setItem('hardPhyDrafts', JSON.stringify(drafts));
...
function submit() {
  ...
  .then((result) => {
    if (result.isConfirmed) {
      ...
      Swal.fire(`Your score: ${score} out of 30 and your badge is ${overallBadge}`)
        .then(() => {
...
function submitDraftQuestions(originalScore) {
  ...
  Swal.fire(`Your total score: ${totalScore} out of 30 and your badge is ${badge}`);
  setTimeout(() => {
    window.location.href = "/options_page/physics/index.html";
  }, 3000);
}
