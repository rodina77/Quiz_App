...
function loadQuiz() {
  fetch("/physics/physicsQuiz.json")
    ...
    .then((data) => {
      ...
      const drafts = JSON.parse(localStorage.getItem('randomPhyDraft')) || [];
      const allQuestions = [...data.EasyPhysics, ...data.IntermediatePhysics]; 
...
function draft(buttonElement, name, question, answer1, answer2, answer3) {
  let drafts = JSON.parse(localStorage.getItem('randomPhyDraft')) || [];
  ...
  localStorage.setItem('randomPhyDraft', JSON.stringify(drafts));
